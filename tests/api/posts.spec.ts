...
test.describe('API Posts - CRUD', () => {
    test('GET /posts - Listar todos os posts - status 200', async ({ request }) => {
        ...
        const primeiroPost = body[0] as { id?: number; title?: string; userId?: number };
        expect(primeiroPost).toHaveProperty('id');
        expect(primeiroPost).toHaveProperty('title');
        expect(primeiroPost).toHaveProperty('userId');
    });

    test('GET /posts/1 - Buscar post por ID - status 200', async ({ request }) => {
    ...
    test('POST /posts - Criar novo post - status 201', async ({ request }) => {
        ...
        const body = (await response.json()) as { id?: number; title?: string; body?: string };
